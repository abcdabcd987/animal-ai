...
public:
    ...
    void position(const Point& pos) { _pos = pos; }
    ...
    int manhattanDistance(const Point& target) const;
    int manhattanDistance(const Animal* target) const;
};
...
inline int Animal::manhattanDistance(const Point& target) const {
    return abs(target.x-_pos.x) + abs(target.y-_pos.y);
}

inline int Animal::manhattanDistance(const Animal* target) const {
    return manhattanDistance(target->position());
}
